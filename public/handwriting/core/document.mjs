...
if (!s || typeof s.id !== 'string' || ids.has(s.id) || s.id.length > 100 || !BRUSHES.includes(s.tool) || !['pen', 'touch', 'mouse'].includes(s.input) || !/^#[0-9a-f]{6}$/i.test(s.color) || !finite(s.size, 1, Math.max(60, BRUSH_SIZES[s.tool]?.max ?? 0)) || !(s.brushVersion === 1 || (['crayon', 'pencil'].includes(s.tool) && s.brushVersion === 4) || (['pencil', 'marker', 'watercolor', 'crayon'].includes(s.tool) && s.brushVersion === 2) || (['pencil', 'watercolor', 'crayon'].includes(s.tool) && s.brushVersion === 3)) || (s.opacity !== undefined && !finite(s.opacity, 0, 1)) || !finite(s.smoothingMs, 0, 100) || !Array.isArray(s.points) || !s.points.length) fail();
